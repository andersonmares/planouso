 ...
 */
jQuery.fn.carregarMunicipios = function (selector, settings) {
    var config = {
        method     : "GET",
        url        : "",
        type       : "json",
        ...
        success    : function (result) {
            ...
            if (result.status == false) {
                ...
                $.each(result.data, function (cod, val) {
                    if (val.id == config.codSelected) {
                        if (val.id == '') {
                            $(selector).html('<option value="' + config.codNotFound + '">- ' + config.msgNotFound + ' -</option>');
                        } else {
                            options += '<option value="' + val.id + '" selected="selected">' + val.noMunicipioAcentuado + '</option>';
//							$(selector).append('<option value="'+cod+'" selected="selected">'+val+'</option>');
                        }
                    } else if (val.id == '') {
                        $(selector).html('<option value="' + config.codNotFound + '">- ' + config.msgNotFound + ' -</option>');
                    } else {
                        options += '<option value="' + val.id + '">' + val.noMunicipioAcentuado + '</option>';
